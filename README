...
Some goals that this fork wishes to accomplish:

1. Remove all built-in generator support and rely on ES6 generators/3rd party shims.
2. Clean up the parser code, add comments.
3. Fix contextual bugs (such as yielding from a scoped for loop)
4. Allow for multiline expressions (if statements, for loops, etc)
5. Add in compile time type checking, or use something like [closure compiler](https://developers.google.com/closure/compiler/)/[Flow](http://flowtype.org)
    This would allow the removal of a lot of helper functions.
6. Remove the support for dashed identifiers as it can cause confusion:
    Example: a-b, should it mean variable a-b, or a minus b?
    We should use the latter.
7. Bring back native bitwise operations syntax. (<<,>>,|,etc)
