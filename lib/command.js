(function () {
  "use strict";
  var __create, __generatorToPromise, __import, __isArray, __owns, __slice,
      __toArray, __typeof, _once, child_process, fs, gorilla, path, util;
  ...
  child_process.exec("which gjs", (_once = false, function (err, whichGjsStdout, whichGjsStderr) {
    var argv, filenames, hasGjs, optimist;
    if (_once) {
    ...
    argv = optimist.argv;
    function readStdin() {
      var buffer, bufferSize, bytesRead;
      bufferSize = 256;
      buffer = new Buffer(bufferSize);
      while (true) {
        try {
          bytesRead = fs.readSync(process.stdin.fd, buffer, bufferSize);
        } catch (e) {
          if (e.code === "EAGAIN") {
            bytesRead = process.exit(1);
          } else if (e.code === "EOF") {
            break;
          } else {
            throw e;
      ...
      return buffer.toString(null, 0, bytesRead);
    }
    filenames = argv._;
    function main() {
      var _arr, _arr2, _i, _len, _ref, _time, baseFilenames, code, compileTime,
          filename, handleQueue, i, input, item, newArgv, options, replOpts,
      ...
      function handleCode(code) {
        ...
        if (argv.ast) {
          ast = gorilla.ast(code, options);
          result = util.inspect(ast.node, false, null);
        } else if (argv.parse) {
          nodes = gorilla.parse(code, options);
          result = util.inspect(nodes.result, false, null);
        } else if (argv.stdout) {
          compiled = gorilla.compile(code, options);
          if (options.uglify) {
            process.stdout.write("\n");
          }
          result = compiled.code;
        } else if (argv.gjs) {
          compiled = gorilla.compile(code, __import({ "eval": true }, options));
          ...
          setTimeout(
            function () {
              return gjs.stdin.end();
            },
            50
          );
          result = "";
        } else if (argv["eval"]) {
          evaled = gorilla["eval"](code, options);
      ...
      if (argv["eval"] != null) {
        return handleCode(String(argv["eval"]));
      }
      if (argv.stdin) {
        code = readStdin();
        return handleCode(String(code));
      }
      if (!filenames.length) {
        throw new Error("Expected at least one filename by this point");
      }
      if (!argv.compile) {
        input = fs.readFileSync(filenames[0]);
        ...
        return handleCode(String(input));
      }
      if (argv.map) {
      ...
      if (filenames.length > 1 && argv.join) {
        ...
        gorilla.compileFile((_ref = __import({}, options), _ref.input = filenames, _ref.output = argv.output, _ref));
        ...
        for (_arr = __toArray(filenames), _i = 0, _len = _arr.length; _i < _len; ++_i) {
          ...
          _time = new Date().getTime();
          gorilla.compileFile((_ref = __import({}, options), _ref.input = filename, _ref.output = getJsOutputPath(filename), _ref));
          compileTime = new Date().getTime() - _time;
          process.stdout.write((compileTime / 1000).toFixed(3) + " seconds\n");
        }
      }
            ...
            if (bestName != null) {
              ...
              return __generatorToPromise(function *() {
                ...
                try {
                  ...
                  _time = new Date().getTime();
                  gorilla.compileFile((_ref = __import({}, options), _ref.input = bestName, _ref.output = getJsOutputPath(bestName), _ref));
                  compileTime = new Date().getTime() - _time;
                  process.stdout.write((compileTime / 1000).toFixed(3) + " seconds\n");
                } catch (e) {
