(function (GLOBAL) {
  "use strict";
  var __everyPromise, __generatorToPromise, __import, __isArray, __lte, __num,
      __owns, __promiseLoop, __slice, __strnum, __toArray, __toPromise,
      __typeof, _ref, ast, fetchAndParsePreludeMacros, fs, init,
  ...
  __promiseLoop = function (limit, length, body) {
    ...
    return new Promise(function (fulfill, reject) {
      var done, index, result, slotsUsed;
  ...
  fetchAndParsePreludeMacros = (function () {
    ...
    work = __generatorToPromise(function *(sync) {
      var cachePrelude, errored, parsedPrelude, prelude, preludeCacheStat,
          preludeSrcStat;
      preludeSrcStat = yield __toPromise(fs.stat, fs, [preludeSrcPath]);
      try {
        preludeCacheStat = yield __toPromise(fs.stat, fs, [preludeCachePath]);
      } catch (e) {
        if (e.code !== "ENOENT") {
          throw e;
        }
      ...
      if (preludeCacheStat && __lte(preludeSrcStat.mtime.getTime(), preludeCacheStat.mtime.getTime())) {
        cachePrelude = yield __toPromise(fs.readFile, fs, [preludeCachePath, "utf8"]);
        errored = false;
        try {
        ...
        if (errored) {
          yield __toPromise(fs.unlink, fs, [preludeCachePath]);
        }
      }
      if (parsedPreludeMacros == null) {
        prelude = yield __toPromise(fs.readFile, fs, [preludeSrcPath, "utf8"]);
        parsedPrelude = yield parser(prelude, null, { serializeMacros: true, filename: preludeSrcPath });
    ...
    function f() {
      if (parsedPreludeMacros != null) {
        return Promise.resolve(parsedPreludeMacros);
    ...
    exports.getSerializedPrelude = __generatorToPromise(function *() {
      yield f();
      return yield __toPromise(fs.readFile, fs, [preludeCachePath, "utf8"]);
    });
  ...
  exports.parse = __generatorToPromise(function *(source, options) {
    ...
    if (options.macros) {
      ...
      macros = yield fetchAndParsePreludeMacros();
    }
    parseOptions = { filename: options.filename, noindent: !!options.noindent, sync: false, progress: options.progress };
    if (options.embedded) {
    ...
    return yield parser(source, macros, parseOptions);
  });
  exports.getReservedWords = __generatorToPromise(function *(options) {
    if (options == null) {
      options = {};
    }
    if (options.noPrelude) {
      ...
      return parser.getReservedWords(yield fetchAndParsePreludeMacros(true), options);
    }
  });
  ...
  exports.ast = __generatorToPromise(function *(source, options) {
    ...
    if (__isArray(source)) {
      ...
      for (_arr = __toArray(source), _i = 0, _len = _arr.length; _i < _len; ++_i) {
        ...
        array.push(sync ? exports.parseSync(item, options) : yield exports.parse(item, options));
      }
      ...
      parsed = yield exports.parse(source, options);
    }
  ...
  exports.compile = __generatorToPromise(function *(source, options) {
    var compiled, node, startTime, sync, translated;
    if (options == null) {
      options = {};
    }
    sync = options.sync;
    ...
    if (sync) {
      ...
      translated = yield exports.ast(source, options);
    }
  ...
  exports.compileFile = __generatorToPromise(function *(options) {
    var _arr, _i, _len, code, compiled, fileSources, footer, i, input, inputs,
        linefeed, name, node, originalProgress, output, parsed, progressCounts,
        source, sourceMapFile, sources, startParseTime, sync, translated,
        translator;
    if (options == null) {
      options = {};
    }
    ...
    if (sync) {
      ...
      sources = yield __promiseLoop(5, __num(inputs.length), __generatorToPromise(function *(_i) {
        var input;
        input = inputs[_i];
        return yield __toPromise(fs.readFile, fs, [input, "utf8"]);
      }));
    }
    ...
    for (i = 0, _len = sources.length; i < _len; ++i) {
      ...
      if (sync) {
        ...
        _arr.push(yield exports.parse(source, options));
      }
    }
    parsed = _arr;
    if (typeof originalProgress === "function") {
    ...
    yield writeFileWithMkdirp(options.output, code, options.encoding || "utf8");
    if (sourceMapFile) {
      yield writeFileWithMkdirp(sourceMapFile, options.sourceMap.toString(), "utf8");
    }
  });
  ...
  exports["eval"] = __generatorToPromise(function *(source, options) {
    var compiled, result, startTime, sync;
    if (options == null) {
      options = {};
    }
    sync = options.sync;
    ...
    if (sync) {
      ...
      compiled = yield exports.compile(source, options);
    }
    startTime = new Date().getTime();
  ...
  exports.run = __generatorToPromise(function *(source, options) {
    var compiled, mainModule, Module, sync;
    if (options == null) {
      options = {};
    }
    sync = options.sync;
    if (typeof process === "undefined") {
      if (sync) {
        return exports.evalSync(source, options);
      } else {
        return yield exports["eval"](source, options);
      }
    }
  ...
  init = exports.init = __generatorToPromise(function *(options) {
    if (options == null) {
      options = {};
    }
    yield fetchAndParsePreludeMacros();
  });
  ...
  exports.getMtime = __generatorToPromise(function *(source) {
    var _arr, _i, _len, _ref, acc, files, fileStats, fileStatsP, libDir,
        libFile, libFiles, stat, time;
    files = [];
    files.push(path.join(path.dirname(real__filename), "../src/jsprelude.gs"));
    libDir = path.join(path.dirname(real__filename), "../lib");
    libFiles = yield __toPromise(fs.readdir, fs, [libDir]);
