(function () {
  ...
  var __cmp, __create, __import, __in, __isArray, __name, __owns, __slice,
      __toArray, __typeof, _ref, Arguments, Arr, AstTypeToClass, Await, Binary,
      ...
      TryCatch, TryFinally, Unary, util, While, Yield;
  ...
  exports.Binary = Binary = (function (Expression) {
    ...
    function compileAccess(op, left, right, options, level, lineStart, sb) {
      ...
      if (left instanceof Const && typeof left.value === "number") {
        ...
        left.compile(options, 18, lineStart || left instanceof Func && left.promise, sb);
      }
      if (dotAccess) {
        sb(".");
        sb(right.value);
  ...
  exports.Call = Call = (function (Expression) {
    ...
    _Call_prototype.compile = function (options, level, lineStart, sb) {
      ...
      this.func.compile(
        options,
        this.isNew ? 19 : 18,
        lineStart && !wrap && !this.isNew || this.func instanceof Func && this.func.promise,
        sb
      );
      if (!options.minify && this.shouldCompileLarge()) {
        f = compileLarge;
  ...
  function compileFunc(options, sb, name, params, declarations, variables, body, generator, promise) {
  ...
  exports.Func = Func = (function (Expression) {
    ...
    function Func(pos, name, params, variables, body, declarations, generator, promise) {
      var _this;
    ...
    _Func_prototype._toAst = function (pos, ident) {
      ...
      return [
        ...
        [
          Const(pos, this.generator),
          Const(pos, this.promise)
        ]
      );
    };
    Func._fromAst = function (pos, name, params, variables, body) {
      var _i, declarations, generator, promise;
      _i = arguments.length - 2;
      if (_i > 5) {
        declarations = __slice.call(arguments, 5, _i);
      } else {
        _i = 5;
        declarations = [];
      }
      generator = arguments[_i];
      promise = arguments[_i + 1];
      return Func(
        pos,
        name || null,
        params || [],
    ...
    Func.fromJSON = function (line, column, file, name, params, variables, declarations) {
      var _i, body, generator, promise;
      _i = arguments.length - 2;
      if (_i > 7) {
        body = __slice.call(arguments, 7, _i);
      } else {
        _i = 7;
        body = [];
      }
  ...
  exports.Yield = Yield = (function (Expression) {
    var _Expression_prototype, _Yield_prototype;
    function Yield(pos, node, passThru) {
      var _this;
      _this = this instanceof Yield ? this : __create(_Yield_prototype);
      _this.pos = pos;
      _this.node = node;
      _this.passThru = passThru;
      return _this;
    }
    _Expression_prototype = Expression.prototype;
    _Yield_prototype = Yield.prototype = __create(_Expression_prototype);
    _Yield_prototype.constructor = Yield;
    Yield.displayName = "Yield";
    if (typeof Expression.extended === "function") {
      Expression.extended(Yield);
    }
    _Yield_prototype.compile = function (options, level, lineStart, sb) {
      if (level > 2) {
        sb("(");
      }
      sb("yield");
    ...
    _Yield_prototype.compileAsBlock = function (options, level, lineStart, sb) {
      Noop(this.pos).compileAsBlock(options, level, lineStart, sb);
    };
    _Yield_prototype.walk = function (walker) {
      var _ref, node;
      if ((_ref = walker(this.node, this, "node")) != null) {
        node = _ref;
      } else {
      ...
      if (node !== this.node) {
        return Yield(this.pos, node, this.passThru);
      } else {
        return this;
      }
    };
    _Yield_prototype.typeId = 39;
    _Yield_prototype._toAst = function (pos, ident) {
      return [
        this.node.toAst(pos, ident),
        Const(pos, this.passThru)
      ];
    };
    Yield._fromAst = function (pos, node, passThru) {
      return Yield(pos, node, passThru);
    };
    _Yield_prototype._toJSON = function () {
      return this.node.toJSON().concat(this.passThru);
    };
    Yield.fromJSON = function (line, column, file, source) {
      var _i, node, passThru;
      _i = arguments.length - 1;
      if (_i > 4) {
        node = __slice.call(arguments, 4, _i);
      } else {
        _i = 4;
        node = [];
      }
      passThru = arguments[_i];
      return Yield(
        makePos(line, column, file),
        fromJSON(node),
        passThru
  ...
  exports.Await = Await = (function (Expression) {
    var _Await_prototype, _Expression_prototype;
    function Await(pos, node, passThru) {
      var _this;
      _this = this instanceof Await ? this : __create(_Await_prototype);
      _this.pos = pos;
      _this.node = node;
      _this.passThru = passThru;
      return _this;
    ...
    _Expression_prototype = Expression.prototype;
    _Await_prototype = Await.prototype = __create(_Expression_prototype);
    _Await_prototype.constructor = Await;
    Await.displayName = "Await";
    if (typeof Expression.extended === "function") {
      Expression.extended(Await);
    }
    _Await_prototype.compile = function (options, level, lineStart, sb) {
      if (level > 2) {
        sb("(");
      }
      sb("await");
    ...
    _Await_prototype.compileAsBlock = function (options, level, lineStart, sb) {
      Noop(this.pos).compileAsBlock(options, level, lineStart, sb);
    };
    _Await_prototype.walk = function (walker) {
      var _ref, node;
      if ((_ref = walker(this.node, this, "node")) != null) {
        node = _ref;
      } else {
      ...
      if (node !== this.node) {
        return Await(this.pos, node, this.passThru);
      } else {
        return this;
      }
    };
    _Await_prototype.typeId = 40;
    _Await_prototype._toAst = function (pos, ident) {
      return [
        this.node.toAst(pos, ident),
        Const(pos, this.passThru)
      ];
    };
    Await._fromAst = function (pos, node, passThru) {
      return Await(pos, node, passThru);
    };
    _Await_prototype._toJSON = function () {
      return this.node.toJSON().concat(this.passThru);
    };
    Await.fromJSON = function (line, column, file, source) {
      var _i, node, passThru;
      _i = arguments.length - 1;
      if (_i > 4) {
        node = __slice.call(arguments, 4, _i);
