(function (GLOBAL) {
  "use strict";
  var __bind, __cmp, __compose, __create, __curry, __import, __in, __isArray,
      __owns, __slice, __toArray, __typeof, _Block, _DefineMacro, _FunctionBody,
      _FunctionDeclaration, _IdentifierOrAccess, _Name, _o, _ref, _ref2,
      _SomeEmptyLinesWithCheckIndent, _Symbol, addParamToScope,
      ...
      AstStatement, AsyncBody, AsyncBodyNoEnd, AtSignChar, BackslashChar,
      ...
      FunctionType, GeneratorBody, GeneratorBodyNoEnd, getPackageVersion,
      ...
      Parenthetical, Parser, ParserError, ParserNode, PercentSign,
      ...
      quote, RadixNumber, RegexLiteral, requireParameterSequence, Scope,
  ...
  AsyncBody = makeAlterStack("inPromise", true)(Body);
  AsyncBodyNoEnd = makeAlterStack("inPromise", true)(BodyNoEnd);
  ...
  _ref = makeAlterStack("inPromise", false)(_FunctionBody);
  FunctionBody = makeAlterStack("inGenerator", false)(_ref);
  ...
  _FunctionDeclaration = (function () {
    ...
    function getBodyRule(generator, promise) {
      var result;
      result = _FunctionBody;
      result = makeAlterStack("inGenerator", generator)(result);
      return result = makeAlterStack("inPromise", promise)(result);
    }
    return allowSpaceBeforeAccess(function (parser, index) {
      ...
      body = getBodyRule(flags.value.generator, flags.value.promise)(parser, asType.index);
      if (!body) {
        parser.popScope();
        return;
      }
      func = LInternalCall(
        ...
        LValue(index, flagsValue.promise)
      );
  ...
  function RootInner(parser, index) {
    ...
    parser.clearCache();
    return Box(currentIndex, _BlockMutator(result, parser, index));
  }
  ...
  function EmbeddedRootInner(parser, index) {
    ...
    parser.clearCache();
    return Box(currentIndex, LInternalCall("block", index, parser.scope.peek(), nodes.concat([
  ...
  function Root(parser) {
    var _arr, _i, _len, bom, empty, emptyAgain, endSpace, importFile, imports,
        root, shebang;
    bom = BOM(parser, 0);
    ...
    if (Eof(parser, empty.index)) {
      return Box(empty.index, LInternalCall(
        "root",
        empty.index,
        parser.scope.peek(),
    ...
    for (_arr = __toArray(imports.value), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      importFile = _arr[_i];
      parser.clearCache();
      parser["import"](importFile, imports.index);
    }
    parser.clearCache();
    root = RootInner(parser, empty.index);
    parser.clearCache();
    if (!root) {
      return;
    }
    ...
    parser.clearCache();
    return Box(endSpace.index, LInternalCall(
      "root",
      empty.index,
      parser.scope.peek(),
      LValue(empty.index, parser.options.filename || null),
  ...
  function EmbeddedRoot(parser) {
    var bom, root, shebang;
    bom = BOM(parser, 0);
    shebang = Shebang(parser, bom.index);
    parser.clearCache();
    root = EmbeddedRootInner(parser, shebang.index);
    parser.clearCache();
    if (!root) {
      return;
    }
    return Box(root.index, LInternalCall(
      "root",
      shebang.index,
      parser.scope.peek(),
      ...
      LValue(shebang.index, parser.inPromise.peek())
    ));
  }
  function EmbeddedRootGenerator(parser) {
    var result;
    parser.inGenerator.push(true);
    result = EmbeddedRoot(parser);
  ...
  Parser = (function () {
    ...
    _Parser_prototype["import"] = function (filename, index) {
      var fs, fullFilename, parseOptions, path, result, source;
      fs = require("fs");
      path = require("path");
      ...
      fullFilename = path.resolve(path.dirname(this.options.filename), filename);
      source = fs.readFileSync(fullFilename, "utf8");
      parseOptions = { filename: fullFilename, noindent: this.options.noindent };
      result = parse(source, this.macros, parseOptions);
    ...
    _Parser_prototype.macroExpand1 = function (node) {
      ...
      if (node._macroExpanded != null) {
        ...
        while (node instanceof ParserNode.MacroAccess) {
          ...
          this.inPromise.push(node.inPromise);
    ...
    _Parser_prototype.macroExpandAllPromise = function (node) {
      return Promise.resolve(this.macroExpandAll(node));
    };
  ...
  function parse(source, macros, options) {
    var endExpandTime, endParseTime, endReduceTime, expanded, getPosition,
        parser, reduced, result, rootRule, startTime;
    if (macros == null) {
      macros = null;
    }
    if (options == null) {
    ...
    expanded = parser.macroExpandAll(result.value);
