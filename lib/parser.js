(function (GLOBAL) {
  "use strict";
  var __bind, __cmp, __compose, __create, __curry, __generatorToPromise,
      __import, __in, __isArray, __owns, __slice, __toArray, __toPromise,
      __typeof, _Block, _DefineMacro, _FunctionBody, _FunctionDeclaration,
      _IdentifierOrAccess, _Name, _o, _ref, _ref2,
  ...
  RootInnerP = __generatorToPromise(function *(parser, index) {
  ...
  EmbeddedRootInnerP = __generatorToPromise(function *(parser, index) {
  ...
  RootP = __generatorToPromise(function *(parser) {
    ...
    for (_arr = __toArray(imports.value), _i = 0, _len = _arr.length; _i < _len; ++_i) {
      ...
      if (parser.options.sync) {
        ...
        yield parser["import"](importFile, imports.index);
      }
    }
    parser.clearCache();
    if (parser.options.sync) {
      ...
      root = yield RootInnerP(parser, empty.index);
    }
    parser.clearCache();
    if (!root) {
      return;
  ...
  EmbeddedRootP = __generatorToPromise(function *(parser) {
    ...
    if (parser.options.sync) {
      ...
      root = yield EmbeddedRootInnerP(parser, shebang.index);
    }
    parser.clearCache();
    if (!root) {
      return;
  ...
  EmbeddedRootGeneratorP = __generatorToPromise(function *(parser) {
    var result;
    parser.inGenerator.push(true);
    if (parser.options.sync) {
      result = EmbeddedRootP.sync(parser);
    } else {
      result = yield EmbeddedRootP(parser);
    }
  ...
  Parser = (function () {
    ...
    _Parser_prototype["import"] = __generatorToPromise(function *(filename, index) {
      ...
      if (this.options.sync) {
        ...
        source = yield __toPromise(fs.readFile, fs, [fullFilename, "utf8"]);
      }
      parseOptions = { filename: fullFilename, noindent: this.options.noindent, sync: this.options.sync };
      if (this.options.sync) {
        result = parse.sync(source, this.macros, parseOptions);
      } else {
        result = yield parse(source, this.macros, parseOptions);
      }
  ...
  parse = __generatorToPromise(function *(source, macros, options) {
    ...
    expanded = yield parser.macroExpandAllPromise(result.value);
