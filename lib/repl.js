(function (GLOBAL) {
  "use strict";
  var __cmp, __in, __isArray, __owns, __slice, __toArray, __typeof, _i, backlog,
      child_process, enableColors, g, gorilla, module, nonContextGlobals,
      readline, sandbox, stdin, stdout, toGsIdent, toJsIdent, util, vm;
  ...
  exports.start = function (options) {
    ...
    repl.on("line", function (buffer) {
      ...
      try {
        if (pipe) {
          compiled = gorilla.compile(code, { "eval": true, filename: "repl", modulename: "repl" });
          return pipe.stdin.write(compiled.code);
        } else if (options.stdout) {
          compiled = gorilla.compile(code, { bare: true, "return": true, filename: "repl", modulename: "repl" });
          process.stdout.write(compiled.code + "\n");
          return repl.prompt();
        } else if (options.ast) {
          ret = gorilla.ast(code, { sandbox: sandbox, filename: "repl", modulename: "repl" });
          process.stdout.write(util.inspect(ret.node, false, 2, enableColors) + "\n");
          return repl.prompt();
        } else if (options.parse) {
          ret = gorilla.parse(code, { sandbox: sandbox, filename: "repl", modulename: "repl" });
        ...
        process.stderr.write(String(typeof err !== "undefined" && err !== null && err.stack || err) + "\n");
        return repl.prompt();
      }
    });
    repl.setPrompt("gs> ");
